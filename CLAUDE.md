...
│   ├── post-mortem.md      # Lessons from the previous FC4 attempt — read for context
...
Before adding any mod, verify it fits within these limits. They are not targets — they are enforced limits:

- **Server RAM:** ≤ 10 GB
- **Client RAM:** ≤ 6 GB
- **Client FPS:** ≥ 60 FPS baseline (including shaders)
- **Mod count:** ≤ 120 mods including deps (soft cap, enforced with intent)
...
9. Add batch multiplayer tests to `docs/testing/multiplayer.md`
10. Bump version for each fix made during testing — `0.N.2`, `0.N.3`, etc.
11. Once SP testing passes, merge to `main`
12. **Pull on server and run server test pass — this is not optional.** Multiplayer testing is required before a batch is considered done.
...
- `id` — 5-digit zero-padded integer, incremented per file regardless of status (next: check the highest existing ID and add 1; starts at 00001)
...
3. Note any auto-pulled dependencies — they count toward the mod total and the mod budget
4. Check the running mod count against the ≤120 budget before adding
5. Run a compat pass each batch (EMI addons, Curios, etc.)
6. Update `docs/watchlist.md` for anything deferred
7. Update `docs/charter.md` issue log for any surprises found during adds
8. Commit mod manifests + doc changes together with a clear `feat(batchN):` message
9. Create the batch test file before handing off for testing
